...
public class UserHub : Hub<IHubResponses>
{
    ...
    public async Task<GroupsName> JoinGroup(JoinGroupRequest joinGroupRequest)
    {
        try
        {
            ...
            {
                // Atualiza no Grupo de Conexões Ativas
                var userToRemove = _activeUserService.UpdateUserConnectionId(joinGroupRequest.userId, joinGroupRequest.groupName, callerConnectionId);

                // Atualiza no "Groups" do SignalR
                if (userToRemove != null)
                {
                    await Groups.RemoveFromGroupAsync(userToRemove.connectionId, joinGroupRequest.groupName.ToString());
            ...
            else
            {
                ...
                // Se o usuário não está no grupo, insira a ConnectionId ao grupo
