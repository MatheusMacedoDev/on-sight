...
const RenderCalled = ({ detailedItem, setDetailedItem, user, toast, sendNewCallAlertToTechnician, onClose, connection }) => {
  const [isLoadingApprove, setIsLoadingApprove] = useState(false);
  const [isLoadingDisapprove, setIsLoadingdisapprove] = useState(false);

  const { changed, setChanged } = useContext(UserContext);
  ...
  const handleUpdateAttendantRevision = async (isApproved) => {
    ...
    if (!detailedItem.urgencyStatusId) {
        toast({
          title: "Erro ao processar chamado",
          description:
            "Não foi possível realizar a operação, pois não foi definida um status de urgência",
        });
        return;
    }

    if (!detailedItem.deadLine) {
        toast({
          title: "Erro ao processar chamado",
          description:
            "Não foi possível realizar a operação, pois não foi definida uma data de entrega",
        });
        return;
    }
    
    if (!detailedItem.instantSolutionDescription && !isApproved) {
        toast({
          title: "Erro ao processar chamado",
          description:
            "Não foi possível realizar a operação, pois não é possível reprovar chamado sem uma solução imediata",
        });
        return;
    }
  ...
  return (
    <div className="flex flex-col justify-between h-auto gap-8">
      ...
      <div className="flex justify-between">
        <button
          onClick={async () => {
            setIsLoadingdisapprove(true);

            await handleUpdateAttendantRevision(false);

            setIsLoadingdisapprove(false);
          }}
        ...
        <button
          onClick={() => {
            ...
            handleUpdateAttendantRevision(true);
  ...
  const renderServiceCallDetails = () => (
    <div className="flex flex-col justify-between h-full gap-4">
      ...
      <div>
        ...
        {detailedItem.instantSolution && (
          <label className="flex items-center font-nunito-bold text-[#142937]">
            Solução imediata:
            <input
              type="text"
              name="urgencyStatusId"
              value={detailedItem.instantSolution}
              className="font-nunito-light text-[#142937] ml-1 p-1 bg-[transparent]"
              disabled
            />
          </label>
    ...
    >
      ...
      <div className="p-8 overflow-y-scroll h-[81%]">
        {loading ? (
          ...
          detailedItem.callStatusId == 0 ? (
            <RenderCalled detailedItem={detailedItem} onClose={onClose} setDetailedItem={setDetailedItem} user={user} toast={toast} sendNewCallAlertToTechnician={sendNewCallAlertToTechnician} />
