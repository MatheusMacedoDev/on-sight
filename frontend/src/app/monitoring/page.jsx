...
import { RealTimeContext } from "@/providers/realtimeContext";
...
const Monitoring = () => {
  ...
  // const { connection, setConnection } = useContext(RealTimeContext); // Controla a conexão em tempo real
  const [connection, setConnection] = useState(null); // Controla a conexão em tempo real
  ...
  const initRealtimeConnection = useCallback(async (connectionBuilder) => {
  ...
  useEffect(() => {
    ...
    initRealtimeConnection(connectionBuilder);

    return () => {
      document.head.removeChild(script);
      if (connection) {
        connection.stop().then(() => console.log("Connection stopped"));
      } else if (connectionBuilder) {
        connectionBuilder.stop().then(() => console.log("Connection stopped"));
      }
    };
  }, []);
          ...
          <div className={`flex gap-2 absolute z-40 p-4 transition transform ${isModalOpen ? 'translate-x-[25vw]' : ''} duration-300`}>
            {
              ...
              mostRecentMetricsOfCategories.map(metric =>
                <KpiCard
                  ...
                  metricValue={metric.metricCategoryName == 'Taxa de recorrência' ? metric.value * 100 : metric.value}
                  metricUnit={metric.metricUnit}
                  deltaPercentage={metric.percentualDelta.toFixed(3)}
